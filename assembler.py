        ...
        if instructions[PC][0] == '.ORG':
            ...
            while (PC < len(instructions) and instructions[PC][0] != '.ORG'):
                ...
                if not opCodes.__contains__(instructions[PC][0]):
                    Memory[key] += bin(int(instructions[PC]
                                       [0], 16))[2:].zfill(32)
                else:
                    ...
                    if instructions[PC][0] in oneOperand:
                        ...
                        Sources = instructions[PC][1].split(',')
                        Memory[key] += registers[Sources[1]]
                        Memory[key] += registers[Sources[0]]
                    elif instructions[PC][0] in dstAndsrc1Andsrc2:
                        Sources = instructions[PC][1].split(',')
                        Memory[key] += registers[Sources[0]]
                        Memory[key] += registers[Sources[1]]
                        Memory[key] += registers[Sources[2]]
                    elif instructions[PC][0] in immOnly:
                        Memory[key] += '0' * 9 # to fil rdst, rsrc1, rsrc2
                        Memory[key] += bin(int(instructions[PC][1], 16))[2:].zfill(16)
                    elif instructions[PC][0] in SpecialCases:
                        specialCaseHandler(key, instructions[PC])
                    else:
                        raise Exception("Error: Instruction not found")
...
def specialCaseHandler(key: int, instruction):
    source = instruction[1].split(',')
    if(instruction[0] == 'IADD'):

        # Memory[key] += '0' * 9
        # Memory[key] += bin(int(instruction[1], 16))[2:].zfill(32)
        Memory[key] += registers[source[0]]
        Memory[key] += registers[source[1]]
        Memory[key] += '0' * 3  # to fill src2
        # convert source[2] from Hexa to binary and add it to the memory
        Memory[key] += bin(int(source[2], 16))[2:].zfill(16)
    elif instruction[0] == 'LDM':
        Memory[key] += registers[source[0]]
        Memory[key] += '0' * 6  # to fill src1, src2
        Memory[key] += bin(int(source[1], 16))[2:].zfill(16)
    elif instruction[0] == 'LDD':
        rdst = source[1].split('(')[1].split(')')[0]
        offset = source[1].split('(')[0]
        Memory[key] += registers[source[0]]
        Memory[key] += registers[rdst]
        Memory[key] += '0' * 3  # to fill src2
        Memory[key] += bin(int(offset, 16))[2:].zfill(16)
    elif instruction[0] == 'STD':
        Memory[key] += '0' * 3  # to fill dst
        Memory[key] += registers[source[0]]
        rdst = source[1].split('(')[1].split(')')[0]
        offset = source[1].split('(')[0]
        Memory[key] += registers[rdst]
    ...
    # check if the instruction is 32 bits
    if(len(Memory[key]) != 32):
        raise Exception(f"Memory[{key}] is not 32 bits")
...
def assemble(fileName):
    ...
    # open the file
    file = open("./TestCases/" + fileName + ".asm", "r")
